...
void *List_first(List *pList)
{
    ...
    // printf("\n in list first %p", pList->current_item->item);
    // printf("inside first\n");
    fflush(stdout);
...
int List_append(List *pList, void *pItem)
{
    ...
    if (countOfNodes < LIST_MAX_NUM_NODES)
    {
        ...
        if (pList->head == NULL)
        {
            ...
            pList->count++;
            countOfNodes++;
            // printf("\n going here first %d at tail and head %d ",*((int*)pList->head->item));
            return LIST_SUCCESS;
        }
        ...
        else
        {
            ...
            // printf("\n going here second %d  %d",*((int*)pList->tail->item));
            return LIST_SUCCESS;
        }
    }
    return LIST_FAIL;
...
int List_prepend(List *pList, void *pItem)
{
    ...
    if (countOfNodes < LIST_MAX_NUM_NODES)
    {
        ...
        if (pList->head == NULL)
        {
            ...
            pList->count++;
            countOfNodes++;
            // printf("look herein prepend inside if %p \n",pList->head->item);
        }
        else
        {
            Node *temp = pList->head;
        ...
        pList->state = 2;
        Node *cur = pList->current_item;
        // printf("\n going here prepend last %d  %d %p %p\n", *((int *)pList->head->item), *((int *)(pList->current_item->item)), pList->current_item->item, pList->current_item);
        return LIST_SUCCESS;
    }
    printf("\n failed to prepend\n");
...
{
    printf("removing from list pid %d\n",*((int*)pList->current_item->item));
    fflush(stdout);
